...
function create(customConfig, platform) {
    ...
    const device = {};
    const { utilities, variants } = (0, plugin_1.getPluginFunctions)(config.plugins);
    ...
    function style(...inputs) {
        ...
        for (const utility of utilities) {
            ...
            if (!styleIr) {
                const parser = new UtilityParser_1.default(utility, config, cache, device, platform, variants);
    ...
    tailwindFn.prefixMatch = (...prefixes) => {
        ...
        const parser = new UtilityParser_1.default(`${joined}:flex`, config, cache, device, platform, variants);
